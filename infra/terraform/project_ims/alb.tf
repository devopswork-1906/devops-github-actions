...
module "alb" {
  ...
  source     = "../../../modules/tf-aws-module-alb"
  depends_on = [module.vpc, module.asg, module.acm]
  # Core ALB settings
  name                       = local.alb_name                    # Unique ALB name (per environment)
  ...
  subnets                    = module.vpc.public_subnets         # ALB subnets
  ...
  listeners = {
    ...
    https = {
      ...
      certificate_arn             = module.acm["mockdns"].acm_certificate_arn
      additional_certificate_arns = [module.acm["additional"].acm_certificate_arn]
      forward = {
  ...
  tags = merge(var.tags["alb_tags"], var.tags["common_tags"])
}
