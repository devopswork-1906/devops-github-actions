...
resource "aws_route_table" "public" {
  ...
  tags = merge(
    ...
    {
      "Name" = format("%s-rt-${var.public_subnet_suffix}", var.name)
    },
  )
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    ...
    {
      "Name" = var.single_nat_gateway ? "${var.name}-rt-${var.private_subnet_suffix}" : format(
        "%s-rt-${var.private_subnet_suffix}-%s",
        var.name,
...
resource "aws_route_table" "database" {
  ...
  tags = merge(
    ...
    {
      "Name" = var.single_nat_gateway || var.create_database_internet_gateway_route ? "${var.name}-rt-${var.database_subnet_suffix}" : format(
        "%s-rt-${var.database_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
...
################################################################################
# Extra Routes aws_route_table.public[0].id
################################################################################
...
resource "aws_route" "database_route_table_routes" {
  for_each                  = var.private_route_table_routes
  route_table_id            = aws_route_table.database[0].id
  destination_cidr_block    = lookup(each.value, "destination_cidr_block", null)
  egress_only_gateway_id    = lookup(each.value, "egress_only_gateway_id", null)
